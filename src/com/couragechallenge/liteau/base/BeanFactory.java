 ...
 * 
 * editor: huwei 20131219,modification for the project demand, but Copy right still reserved by Liang Fei;
 * 
 */
public class BeanFactory {
	...
	public static <T> T createBean(Class<T> beanClass, Properties properties) {
		...
		return createBean(beanClass,name,properties);
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static <T> T createBean(Class<T> beanClass, String name, Properties properties) {
						...
						if (value != null && value.trim().length() > 0) {
							...
							if (parameterType.isArray()) {
								...
								for (int i = 0; i < values.length; i++) {
									Object o = parseValue(property, key+"["+i+"]", values[i], componentType, properties, caches, instances, inits);
									if (o != null) {
										objs[idx ++] = o;
									}
								}
...
//							}
	...
	private static Properties getProperties() {
		...
		//		List<Map<String,String>> list = DAOHelper.query("select * from bean");
		//		Map<String,String> map = DAOHelper.list2Map(list,"name","value");
		//		if(map.size()>0){
		//			p.putAll(map);	
		//		}
		p.put("queryListActivity1", "#Activity");
		p.put("queryListActivity1.layoutId", "list_fragment");
		p.put("queryListActivity1.fragmentViews", "listfragment");
		p.put("queryListActivity1.fragments", "cn.com.egova.egovamobile.test.BListFragment");
		p.put("queryListActivity1.fragments[0].filter", "cn.com.egova.egovamobile.eventquery.QueryListFilter");
		p.put("queryListActivity1.fragments[0].baseListAdapter", "cn.com.egova.egovamobile.eventquery.QueryListAdapter");
		p.put("queryListActivity1.fragments[0].baseListAdapter.context", "#Activity");
		p.put("queryListActivity1.fragments[0].title", "查询列表");
		p.put("queryListActivity1.fragments[0].showBack", "true");
		p.put("queryListActivity1.fragments[0].showFilter", "true");
		//			p.put("queryListActivity.fragment.query", "cn.com.egova.egovamobile.eventquery.QueryListAdapter");
		//			p.put("queryListActivity.fragment.mapProcessor", "cn.com.egova.egovamobile.eventquery.QueryListAdapter");
	...
	public static <T> T createBean(String name){
		return createBean(name,getProperties());
	}
	
	public static <T> T createBean(String name,Properties properties){
		...
		@SuppressWarnings("unchecked")
		Class<T> beanClass = (Class<T>) ClassUtils.forName(value);
		
		return createBean(beanClass, name, properties);		
	}
	
	public static void injectBean(Object instance){
		injectBean(instance,instance.getClass().getSimpleName(),getProperties());
	}
	
	public static void injectBean(Object instance,String name){
		injectBean(instance,name,getProperties());
	}
	
	public static void injectBean(Object instance,Properties properties){
		injectBean(instance,instance.getClass().getSimpleName(),properties);
	}
	
	@SuppressWarnings("unchecked")
	public static void injectBean(Object instance,String name,Properties properties){
		...
		try {
			...
			instances.put(key + "=" + value, instance);
			...
			try {
				...
				throw new IllegalStateException("Failed to inoke inited() in bean class " + cls.getName() + ", cause: " + e.getMessage(), e);
			}
		}
		
	}
	
	public static void injectBean(Object bean, Bundle bundle) {
		if(bean == null){
			return ;
		}
		...
		try {
			...
			for (Method method : methods) {
				String name = method.getName();
				if (name.length() > 3 && name.startsWith(SET_METHOD) && Modifier.isPublic(method.getModifiers())
						&& !Modifier.isStatic(method.getModifiers()) && method.getParameterTypes().length == 1) {
					Class<?> parameterType = method.getParameterTypes()[0];
					String property = name.substring(3, 4).toLowerCase() + name.substring(4);
					Object obj = bundle.getSerializable(property);
					if (null == obj) {
						obj = bundle.getSerializable(property.toUpperCase());
					}
					if (null != obj && parameterType.isAssignableFrom(obj.getClass())) {
			...
			throw new IllegalStateException("Failed to init properties of bean class " + bean.getClass().getName()
